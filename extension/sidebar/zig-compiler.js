...
var f;
var m = "deflate-raw";
var x = self.DecompressionStream;
try {
    new x(m), f = async (t) => {
        let n = new x(m), e = n.writable.getWriter(), i = n.readable.getReader();
        e.write(t), e.close();
        let c, o = [], s = 0, a = 0, l;
        for (; !(l = await i.read()).done;) c = l.value, o.push(c), s += c.length;
        return o.length - 1 ? (c = new Uint8Array(s), o.map((r) => {
            c.set(r, a), a += r.length;
        }), c) : o[0];
    };
} catch {
}
var _ = new TextDecoder();
var h = (t) => {
    throw new Error("but-unzip~" + t);
};
var E = (t) => _.decode(t);
var A = (t) => {
    let n = t.length - 20, e = Math.max(n - 65516, 2);
    for (; (n = t.lastIndexOf(80, n - 1)) !== -1 && !(t[n + 1] === 75 && t[n + 2] === 5 && t[n + 3] === 6) && n > e;);
    return n;
};
function* C(t, n = f) {
    let e = A(t);
    e === -1 && h(2);
    let i = (r, d) => t.subarray(e += r, e += d), c = new DataView(t.buffer, t.byteOffset), o = (r) => c.getUint16(r + e, true), s = (r) => c.getUint32(r + e, true), a = o(10);
    for (a !== o(8) && h(3), e = s(16); a--;) {
        let r = o(10), d = o(28), g = o(30), y = o(32), b = s(20), w = s(42), p = E(i(46, d)), D = E(i(g, y)), L = e, u;
        e = w, u = i(30 + o(26) + o(28), b), yield { filename: p, comment: D, read: () => r & 8 ? n(u) : r ? h(1) : u }, e = L;
    }
}

// src/index.ts
async function buildDirectoryTree(files) {
        ...
        parts.forEach(async (part, index) => {
            if (part !== "") {
                if (index === parts.length - 1) {
                    ...
                    currentDirectory = currentDirectory.contents.get(parts[index]);
                }
            }
        });
    }
    return root.contents.get("std");
}
async function unZipStdLib(source) {
    return await buildDirectoryTree(C(source));
}
async function initZigWASI(std, zigCode, stderrBuffer, debug2 = false) {
    const args = [
        "zigc.wasm",
        "build-exe",
        "input.zig",
        "-Dtarget=wasm32-freestanding",
        "-rdynamic", // Ensure exported functions are visible in WASM
        "-fno-llvm",
        "-fno-lld",
        "-O",
        "ReleaseSmall"
    ...
    const fds = [
        ...
        ConsoleStdout.lineBuffered((msg) => {
            ...
            console.log(`[WASI stderr] ${msg}`);
        }),
        // stderr
        new PreopenDirectory(
            ".",
      /* @__PURE__ */ new Map([["input.zig", new File(new TextEncoder().encode(zigCode))]])
        ),
        new PreopenDirectory("/lib", /* @__PURE__ */ new Map([["std", await unZipStdLib(std)]])),
        new PreopenDirectory("/cache", /* @__PURE__ */ new Map())
    ];
    return new WASI(args, env, fds, { debug: debug2 });
}
async function runZigCompiler(zigc, wasi) {
...
async function runZigOutput(output, wasi) {
...
let stdBufferCache = null;
let compilerBufferCache = null;

export async function compileZigCode(zigCode, statusCallback) {
    if (!stdBufferCache || !compilerBufferCache) {
        if (statusCallback) statusCallback('Downloading Dependencies...');
        try {
            const stdUrl = chrome.runtime.getURL('zig/std.zip');
            const compilerUrl = chrome.runtime.getURL('zig/zig_small.wasm');
            ...
            if (!stdRes.ok || !zigcRes.ok) {
                throw new Error("Failed to fetch Zig dependencies from extension.");
            }

            const stdBlob = await stdRes.blob();
            const stdArrayBuf = await stdBlob.arrayBuffer();
            stdBufferCache = new Uint8Array(stdArrayBuf);

            compilerBufferCache = await zigcRes.arrayBuffer();
        } catch (e) {
            console.error(e);
            throw new Error(`Failed to download Zig dependencies: ${e.message}`);
        }
    }
    ...
    await runZigCompiler(compilerBufferCache, wasi);

    const preopenDir = wasi.fds[3];
    const outputWasmFile = preopenDir.dir.contents.get("input.wasm");

    if (!outputWasmFile || !outputWasmFile.data || outputWasmFile.data.length === 0) {
        const stderrStr = stderrBuffer.join("\n");
        throw new Error(`Compilation failed (input.wasm empty or missing).\nCompiler Output:\n${stderrStr}`);
    }

    return outputWasmFile.data;
}
